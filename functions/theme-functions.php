...
if ( !function_exists( 'tz_head_css' ) ) {
    function tz_head_css() {
        ...
        if( empty($bg_img) ) {
            ...
            if( $bg_pos != 'full' ) {
                ...
                $bg_repeat = get_option('tz_default_bg_repeat');
                $bg_color = get_post_meta($postid, 'tz_background_color', true);
                if( empty($bg_color) ) {
                    $bg_color = get_option('tz_default_bg_color');
                }
            ...
            if( $bg_pos != __('Full Screen', 'framework') ) {
                ...
                // Handle the repeat
                $bg_repeat = get_post_meta($postid, 'tz_background_repeat', true);
		...
		$custom_highlight = get_option('tz_highlight_color');
...
if ( !function_exists( 'tz_analytics' ) ) {
...
if ( !function_exists( 'tz_get_posts_related_by_taxonomy' ) ) {
    function tz_get_posts_related_by_taxonomy($post_id, $taxonomy, $args=array()) {
        $query = new WP_Query();
...
if ( !function_exists( 'tz_audio' ) ) {
    function tz_audio($postid, $width = 560) {
...
if ( !function_exists( 'tz_video' ) ) {
    function tz_video($postid, $width = 560) {
...
if ( !function_exists( 'tz_is_multiple' ) ) {
    function tz_is_multiple($number, $multiple) { return ($number % $multiple) == 0; }
}
